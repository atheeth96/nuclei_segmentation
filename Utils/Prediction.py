...
def whole_img_pred2(IMAGE_PATH,img_name,model,print_prompt=False):
...
def whole_img_pred(IMAGE_PATH,img_name,model,print_prompt=False):
...
#         nucei_thresholded=nuclei_temp>nuclei_thresh
    ...
    return dice,nuclei_temp,bound_temp
...
#     selected_ip=imread(path_to_patient_data+'/ROI/'+img_name)
    ...
    comb_img=img_recon_consep(nuc_img,bound_img)
...
def post_process2(nuc_img,bound_img,patient_name,img_name,threshold='otsu',apply_qc=False,print_prompt=False):
...
def img_recon(nuclei,boundary,print_prompt=True):
...
#     nuclei_seeds_ini=ndimage.binary_fill_holes(nuclei_seeds_ini).astype(int)
    area=len(np.where(nuclei_seeds_ini==1)[0])
    total_area=nuclei_seeds_ini.shape[0]*nuclei_seeds_ini.shape[1]
